...
function wpcc_modify_content($content) {
	...
	if ($postid != 0) {
		...
		foreach($comments_merged as $key => $comment){
			...
			/* $re = '/'.preg_quote(preg_replace('/[\s\pL]/', '[\s\W]*?(?:<\/?[^>]*?>)?[\s\W]*?', $key)).'/'; */
			$re = '/'.preg_replace('/\W/', '.*?', utf8_encode($key)).'/';
			$re = preg_replace('/[\.\*\?]+/', '.*?', $re);
			...
			$content = preg_replace($re, '$0<span data-id="'.$comments_merged[$key]['ids'].'" class="comment c'.$comments_merged[$key]['count'].'"></span>', $content);
		}
	}
